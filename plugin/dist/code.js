...
function analyzeDensity(frame) {
    // Initialize 10x10 density grid
    const densityMap = Array(10).fill(0).map(() => Array(10).fill(0));
        ...
        switch (msg.type) {
            ...
            case 'highlight-section': {
                ...
                if (selection.length === 1 && selection[0].type === 'FRAME') {
                    ...
                    const existingHighlights = figma.currentPage.findAll(node => node.type === 'RECTANGLE' && node.name === 'density-highlight');
...
async function analyzeNode(node) {
    ...
    if ('fills' in node && 'characters' in node) {
        const contrastIssues = await checkContrast(node);
    ...
    // Typography checks
    if (node.type === 'TEXT') {
        const typoIssues = checkTypography(node);
    ...
    if ('layoutMode' in node) {
        const layoutIssues = checkLayout(node);
...
async function analyzeNodeRecursive(node) {
...
// ===========================
async function checkContrast(node) {
    const issues = [];
    try {
        // Get text color
        ...
        while (parent && !bgColor) {
            if ('fills' in parent) {
                const parentFills = parent.fills;
                if (Array.isArray(parentFills)) {
                    const solidFill = parentFills.find(f => f.type === 'SOLID');
                    if (solidFill) {
...
async function checkComponent(node) {
    const issues = [];
...
// PROTOTYPE ANALYSIS
// ===========================
async function analyzePrototypes(nodes) {
    const issues = [];
        ...
        if ('reactions' in node && node.reactions && node.reactions.length > 0) {
            node.reactions.forEach(reaction => {
                if (reaction.action && reaction.action.type === 'NODE') {
                    const targetId = reaction.action.destinationId;
                    if (!targetId) {
                        issues.push({
                            type: 'prototype',
                            severity: 'error',
...
// ===========================
async function smartRename(nodeId) {
...
async function applySingleFix(fix) {
    ...
    switch (fix.type) {
        case 'adjust-contrast':
            await fixContrast(node, fix);
            message = '✓ Contrast adjusted';
            break;
        case 'resize-touch-target':
            fixTouchTarget(node, fix);
...
async function fixContrast(node, fix) {
...
function fixTouchTarget(node, fix) {
...
// UTILITY FUNCTIONS
// ===========================
async function selectAndZoomToNode(nodeId) {
    try {
        const node = await figma.getNodeByIdAsync(nodeId);
        if (!node) {
